...
sessionsWidget :: forall t m . MonadWidget t m => SimpleToken 
  -> Id Connection
  -> m (Route t m)
sessionsWidget token conn = do 
  ...
  _ <- widgetHold (pure never) widgetE
  return $ Route never
  where 

  renderSession :: WithId (Id Session) Session -> m (Event t ())
  renderSession (WithField _ sess) = elClass "div" "panel panel-default" $ do 
    elClass "div" "panel-body" $ do
      let start = sess ^. rlens (Proxy :: Proxy '("start", UTCTime)) . rfield
          ...
          log = sess ^. rlens (Proxy :: Proxy '("log", EventLogId)) . rfield
      elAttr "span" [("style", "font-weight: bold;")] $ text $ "Start: " <> show start 
        <> "End: " <> show end

      closeE <- case end of 
        Nothing -> return never
        Just _ -> blueButton "Disconnect"

      viewE <- blueButton "View"
